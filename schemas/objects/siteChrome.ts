...
const logoField = (name: string, title: string, description: string) =>
  defineField({
    ...
    type: "image",
    description:
      `${description} SVG preferred: logos are recoloured through CSS custom properties, which only ` +
    ...
    fields: [
      defineField({
        ...
        type: "string",
        validation: (Rule) =>
          Rule.required().warning("Needed for screen readers."),
      }),
      urlField(
        "href",
        "Link target",
        'Where clicking the logo goes; defaults to "/" when empty.',
      ),
    ],
  });
...
export const navSlot = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: "string",
      description:
        'Name from the app icon set, e.g. "sparkles", "bed", "ticket". Leave empty for a label-only slot.',
      options: {
        list: [
    ...
    defineField({
      ...
      type: "string",
      description:
        "Accent applies the highlighted treatment used for Sweepstakes.",
      options: {
        list: [
...
export const authControls = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      title: "Sign-in label",
      type: "string",
      description: `e.g. "Sign Up". ${INHERITS}`,
    }),
...
export const nexusNavbar = defineType({
  ...
  options: { collapsible: true, collapsed: false },
  description:
    "Legacy nested Nexus data retained until existing documents are migrated to the shared Navbar fields.",
  fields: [
    ...
    defineField({
      ...
      type: "array",
      description:
        `The top-level items. Order here is the order on screen. ${INHERITS} Setting any item replaces ` +
...
export const navbarConfig = defineType({
  ...
  fields: [
    defineField({
      ...
      type: "string",
      description: `Choose which navbar renderer the tenant uses. The navigation content below is shared by both. ${INHERITS}`,
      options: {
        list: [
    ...
    logoField(
      "primaryLogo",
      "Primary logo",
      "Shown at the left of either navbar.",
    ),
    defineField({
      name: "navigation",
      title: "Navigation",
      type: "navigationConfig",
      description:
        "Shared labels, destinations, SVG icons, visibility, and behavior for both navbar renderers.",
    }),
    defineField({
      name: "auth",
      title: "Account control",
    ...
    defineField({
      ...
      deprecated: {
        reason:
          "Use the shared Navbar fields above. Existing data remains readable until migrated.",
      },
    }),
    defineField({
      name: "legacy",
    ...
    defineField({
      ...
      fields: [
        ...
        logoField(
          "logo",
          "Co-branding lockup",
          'The "powered by" mark. Usually supplied by the universal default.',
        ),
      ],
    }),
    defineField({
...
export const siteMetadata = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      validation: (Rule) =>
        Rule.max(70).warning(
          "Titles beyond ~70 characters get truncated in search results.",
        ),
    }),
    defineField({
      name: "titleTemplate",
    ...
    defineField({
      ...
      validation: (Rule) =>
        Rule.max(160).warning(
          "Descriptions beyond ~160 characters get truncated.",
        ),
    }),
    defineField({
      name: "canonicalUrl",
      title: "Canonical URL",
      type: "url",
      description: `Preferred public URL for this page. Leave empty when the application can derive it from the tenant domain and route. ${INHERITS}`,
    ...
    defineField({
      ...
      type: "boolean",
      description:
        "Adds a noindex directive. Useful on staging tenants and unlisted pages.",
      initialValue: false,
    }),
    defineField({
      name: "noFollow",
      ...
      type: "boolean",
      description:
        "Adds a nofollow directive. Use only for pages whose outbound links should not be crawled.",
      initialValue: false,
    }),
  ],
});
